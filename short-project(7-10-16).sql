...
--
-- Host: 127.0.0.1
-- Generation Time: Oct 07, 2016 at 11:08 AM
...
CREATE TABLE IF NOT EXISTS `t_doc_req` (
  ...
  `berkas_upload` varchar(128) NOT NULL,
...
INSERT INTO `t_doc_req` (`id_req`, `kode_berkas`, `berkas_upload`, `date_upload`) VALUES
(27, 'berkas111', '', ''),
(0, 'berkas111', '', ''),
(0, 'berkas111', '', ''),
(28, 'berkas111', '', ''),
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `t_doc_req_data` AS select `t_doc_req`.`id_req` AS `id_req`,`t_doc_req`.`kode_berkas` AS `kode_berkas`,`t_dokumen`.`nama_berkas` AS `nama_berkas`,`t_dokumen`.`kode_skpd` AS `kode_skpd`,`t_doc_req`.`berkas_upload` AS `berkas_upload`,`t_doc_req`.`date_upload` AS `date_upload` from ((`t_doc_req` join `t_dokumen` on((`t_doc_req`.`kode_berkas` = `t_dokumen`.`kode_berkas`))) join `t_request` on((`t_doc_req`.`id_req` = `t_request`.`id`)));
