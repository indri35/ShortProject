...
--
-- Host: 127.0.0.1:3307
-- Generation Time: Oct 13, 2016 at 11:08 PM
...
(29, 50, '55', 'otp.JPG', '2016-10-13'),
(30, 51, '123222', 'ak_tiv.as.JPG', '2016-10-13'),
(31, 52, 'rrrrrr', 'rrr.JPG', '2016-10-13'),
(32, 53, '123222', NULL, ''),
(33, 53, '55', NULL, ''),
(34, 53, 'rrrrrr', NULL, '');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `t_doc_req_data`  AS  select `t_doc_req`.`no_req` AS `no_req`,`t_doc_req`.`id_req` AS `id_req`,`t_request`.`nik_pemohon` AS `nik_pemohon`,`t_doc_req`.`kode_berkas` AS `kode_berkas`,`t_dokumen`.`nama_berkas` AS `nama_berkas`,`t_request`.`tujuan_permohonan_info` AS `tujuan_permohonan_info`,`t_request`.`jenis_request` AS `jenis_request`,`t_dokumen`.`kode_skpd` AS `kode_skpd`,`t_doc_req`.`berkas_upload` AS `berkas_upload`,`t_doc_req`.`date_upload` AS `date_upload` from ((`t_doc_req` join `t_dokumen` on((`t_doc_req`.`kode_berkas` = `t_dokumen`.`kode_berkas`))) join `t_request` on((`t_doc_req`.`id_req` = `t_request`.`id`))) ;
...
--
ALTER TABLE `t_doc_req`
  MODIFY `no_req` int(3) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=35;
--
...
--
ALTER TABLE `t_request`
  MODIFY `id` int(16) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=54;
--
